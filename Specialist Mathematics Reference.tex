...
\newenvironment{note}[1]{%
	\mbox{}\\\tcolorbox[beamer,breakable,%
	title=Note: #1,standard jigsaw,opacityback=0, colframe=blue!75!black]}{\endtcolorbox\mbox{}\\}
	...
	\section{Functions}
		...
		\subsection{Areas}
			\begin{examquestion}{2021 NEAP Methods MCQ20}
				...
				\end{align*}
			\end{examquestion}
		\subsection{Codomain Weirdness}
			\paragraph{The codomain of a function} is the set of possible outputs. For $f: D\to\R$ we say that the codomain is all real numbers. If, for instance, we specify a codomain of a function $g: \R\to\R^+$, we ignore all negative and 0 values the function produces. This differs from range as the Codomain is all values that \textit{could} come out, whereas the range is all values that \textit{do} come out from the function.
	...
	\section{Complex Numbers}
		...
		\subsection{Subsets of Complex Plane}
			\subsubsection{Circles}
				...
				\]
				\paragraph{Maximum Value of Modulus} For points on a circle given by $|z-z_1|=r$ for $z,z_1\in\mathbb{C}$ and $r\in\R$, the maximum value of $|z|$ is given by $|z_1|+|r|$.
			\subsubsection{Lines}
	...
	\section{Differential Calculus}			
		...
		\subsection{Rational Functions}
			\subsubsection{Discontinuity - Vertical Asymptotes vs Holes}
				...
				\begin{examquestion}{2021 VCAA Spesh E2 QB1d.i}
					\begin{leftbar}
						Let $\displaystyle g_k(x)=\frac{(2x-3)(x+5)}{(x-k)(x+2)}$, where $k$ is some real constant.\\
						i) For what values of $k$ will the graph of $g_k$ have two asymptotes?
					\end{leftbar}
					Find all factors that cancel or make the function into an indeterminate form:
					\[
						(x-k)=(x+2)\implies k=-2\qquad (x-k)=(x+5)\implies k=-5\qquad 2(x-k)=2x-3\implies k=\frac{3}{2}
					\]
					\textbf{Note} the multiplication by 2 in the third step so that the coefficients of $x$ match.\\
					Therefore $k\in\{-5,-2,\frac{3}{2}\}$.
				\end{examquestion}
